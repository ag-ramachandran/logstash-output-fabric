...
class LogStashEventsBatcher
    ...
    def initialize(fabricconfiguration)
        ...
        @client = LogAnalyticsClient::new(fabricconfiguration)
    end # initialize
    ...
    def batch_event_document(event_document)
        ...
        if (current_document_size >= @fabricconfiguration.MAX_SIZE_BYTES - 1000)
    ...
    def send_to_fabric_target(call_payload, amount_of_documents)

        retransmission_timeout = Time.now.to_i + @fabricconfiguration.retransmission_time
        api_name = "Logs Ingestion API"
        is_retry = false
        force_retry = false

        while Time.now.to_i < retransmission_timeout || force_retry
            seconds_to_sleep = @fabricconfiguration.RETRANSMISSION_DELAY
            force_retry = false
            # Retry logic:
            ...
            # All other http errors are retried for total every of @fabricconfiguration.RETRANSMISSION_DELAY until @fabricconfiguration.retransmission_time seconds passed
            begin
                @logger.debug(transmission_verb(is_retry) + " log batch (amount of documents: #{amount_of_documents}) to DCR stream #{@fabricconfiguration.dcr_stream_name} to #{api_name}.")
                ...
                if LogAnalyticsClient.is_successfully_posted(response)
                    ...
                    @logger.info("Successfully posted #{amount_of_documents} logs into log analytics DCR stream [#{@fabricconfiguration.dcr_stream_name}] x-ms-request-id [#{request_id}].")
                    return
                else
